<?php
function jkbssteelGetElement_ID($i){
    $id = '';
    if(is_page()){
        $id = get_the_ID();
    }
    $id = 'jkbssteel_element_'.$id.'_00'.$i;
    return $id;
}
function jkbssteelGetElementExtraClass($classes){
    // $classes = filter_var($classes, FILTER_SANITIZE_STRING);
    $classes = wp_strip_all_tags($classes, true);
    return $classes;
}
function jkbssteel_custom_css($css){
    wp_register_style( 'jkbssteel-custom-inline-style', false );
    wp_enqueue_style( 'jkbssteel-custom-inline-style' );
    wp_add_inline_style( 'jkbssteel-custom-inline-style', $css );
}
function jkbssteelLoadSectionCSS($cssID, $argDesktop, $argTab, $argMobile, $customCSS = ''){
    // echo '<pre>';
    // print_r($argDesktop);
    // print_r($argTab);
    // print_r($argMobile);
    ...
    jkbssteel_custom_css($render);
    if($customCSS){
        $customCSS = str_replace("&", '#'.$cssID, $customCSS);
        jkbssteel_custom_css($customCSS);
    }
}
function jkbssteel_get_section_title($title, $align='start'){
    if(!$title){
        return false;
    }
    return '<h2 class="h2 text-'.$align.'">'. $title .'</h2>';
}
